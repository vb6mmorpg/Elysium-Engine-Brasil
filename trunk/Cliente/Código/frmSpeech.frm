...
Begin VB.Form frmSpeech 
   ...
   Begin VB.Label lblWarn 
      ...
      Caption         =   "* É obrigatório ter um para salvar a fala."
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   6.75
         Charset         =   0
...
Dim O As Long
    ...
    For p = 1 To 3
        If frmSpeech.chkRespond.Value = 1 Then
            ...
            frmSpeech.lblGoTo(p - 1).Caption = "Ir para " & Speech(EditorIndex).Num(O).Responces(p).GoTo
        ...
        Else
            ...
            frmSpeech.txtTalk(p - 1).Text = "Escreva uma resposta"
            frmSpeech.scrlGoTo(p - 1).Value = 0
            frmSpeech.lblGoTo(p - 1).Caption = "Ir para 0"
...
Private Sub cmdCancel_Click()
    Call SendData("NEEDSPEECH" & SEP_CHAR & EditorIndex & END_CHAR)
...
Private Sub scrlGoTo_Change(Index As Integer)
    lblGoTo(Index).Caption = "Ir para " & scrlGoTo(Index).Value
End Sub
...
Dim p As Long
    ...
    For p = 1 To 3
        If chkRespond.Value = 1 Then
            ...
            lblGoTo(p - 1).Caption = "Ir para " & Speech(EditorIndex).Num(I).Responces(p).GoTo
