...
Begin VB.Form frmServer 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Tab(0).Control(0)=   "CustomMsg(0)"
      Tab(0).Control(1)=   "Say(0)"
      ...
      Tab(1).Control(0)=   "Command67"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).Control(1)=   "Command45"
      Tab(1).Control(1).Enabled=   0   'False
      Tab(1).Control(2)=   "picJail"
      Tab(1).Control(2).Enabled=   0   'False
      Tab(1).Control(3)=   "picStats"
      Tab(1).Control(3).Enabled=   0   'False
      Tab(1).Control(4)=   "picReason"
      Tab(1).Control(4).Enabled=   0   'False
      Tab(1).Control(5)=   "Command3"
      Tab(1).Control(5).Enabled=   0   'False
      Tab(1).Control(6)=   "Command24"
      Tab(1).Control(6).Enabled=   0   'False
      Tab(1).Control(7)=   "Command23"
      Tab(1).Control(7).Enabled=   0   'False
      Tab(1).Control(8)=   "Command22"
      Tab(1).Control(8).Enabled=   0   'False
      Tab(1).Control(9)=   "Command21"
      Tab(1).Control(9).Enabled=   0   'False
      Tab(1).Control(10)=   "Command20"
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.Label ShutdownTime 
            AutoSize        =   -1  'True
            BackStyle       =   0  'Transparent
            Caption         =   "Desligamento: Não Ativo"
            Height          =   195
            Left            =   1920
            TabIndex        =   31
            Top             =   1200
...
Dim Index As Long, i As Long
    ...
    ElseIf num = 4 Then
        ...
        If txtReason.text = vbNullString Then
            MsgBox "Por favor, digite uma razão!"
            Exit Sub
        End If

        If Index > 0 Then
            If IsPlaying(Index) Then
                Call GlobalMsg(GetPlayerName(Index) & " foi preso pelo servidor! Razão: " & txtReason.text, White)
            End If
            Call PlayerWarp(Index, scrlMap.Value, scrlX.Value, scrlY.Value)
        End If
    End If
...
Dim Index As Long
    ...
    For Index = 1 To MAX_PLAYERS
        ...
        If IsPlaying(Index) = True Then
            ...
            Call SendHP(Index)
            Call PlayerMsg(Index, "Você foi curado pelo servidor!", BrightGreen)
        End If
    Next
End Sub
...
Dim Index As Long
    ...
    If Index > 0 Then
        ...
        End If
        Call AlertMsg(Index, "Você foi kickado pelo servidor!")
    End If
End Sub
...
Dim Index As Long
    ...
    Index = lvUsers.ListItems(lvUsers.SelectedItem.Index).text
    Call PlayerMsg(Index, "Você foi emudecido!", White)
...
Dim Index As Long
    ...
    Index = lvUsers.ListItems(lvUsers.SelectedItem.Index).text
    Call PlayerMsg(Index, "Você não está mais mudo!", White)
    Call TextAdd(frmServer.txtText(0), GetPlayerName(Index) & " não está mais mudo!", True)
...
Private Sub Command28_Click()
    ' Desativado (Motivo: só problemas)
    ...
    For Index = 1 To MAX_PLAYERS
        ...
        If IsPlaying(Index) = True Then
            If GetPlayerAccess(Index) <= 0 Then
                ...
                Call PlayerMsg(Index, "Você foi morto pelo servidor!", BrightRed)
...
Dim i As Long
    ...
    MapInfo(1).Caption = "Revisão: " & Map(Index).Revision
    ...
    MapInfo(7).Caption = "Música: " & Map(Index).Music
...
Dim Index As Long
    ...
    If txtExp.text >= 0 Then
        Call GlobalMsg("O servidor deu " & txtExp.text & " de experiência para todos!", BrightGreen)
        For Index = 1 To MAX_PLAYERS
...
Dim Index As Long
    ...
    If txtReason.text = vbNullString Then
        MsgBox "Por favor, coloque uma razão!"
        Exit Sub
    End If
    Index = lvUsers.ListItems(lvUsers.SelectedItem.Index).text
    ...
    If num = 1 Then
        If Index > 0 Then
            If IsPlaying(Index) Then
                Call GlobalMsg(GetPlayerName(Index) & " foi kickado pelo servidor! Razão: " & txtReason.text, White)
            End If
            Call AlertMsg(Index, "Você foi kickado pelo servidor! Razão: " & txtReason.text)
        End If
    ElseIf num = 2 Then
...
Private Sub Say_Click(Index As Integer)
    ...
    Call TextAdd(frmServer.txtText(0), "Mensagem Rápida: " & Trim$(CMessages(Index + 1).Message), True)
End Sub
...
Dim SaveTime As Long
    ...
    If frmServer.chkChat.Value = Unchecked Then
        ChatSecs = SaveTime
        Label6.Caption = "O Log do Chat está desativado!"
        Exit Sub
    End If

    If ChatSecs <= 0 Then ChatSecs = SaveTime
    If ChatSecs > 60 Then
        Label6.Caption = "Log do Chat salvará em " & Int(ChatSecs / 60) & " minuto(s)"
    Else
        Label6.Caption = "Log do Chat salvará em " & Int(ChatSecs) & " segundo(s)"
    End If
    ChatSecs = ChatSecs - 1
